...
def stock_info_bj_name_code() -> pd.DataFrame:
    ...
    r = requests.post(url, data=payload, headers=extra_utils.get_headers(), proxies=extra_utils.get_proxy(), verify=False, timeout=30)
    data_text = r.text
    ...
    for page in tqdm(range(total_page), leave=False):
        ...
        r = requests.post(url, data=payload, timeout=30, headers=extra_utils.get_headers(), proxies=extra_utils.get_proxy(), verify=False)
        data_text = r.text
        data_json = json.loads(data_text[data_text.find("[") : -1])
...
if __name__ == "__main__":
    ...
    print(stock_info_a_code_name_df.to_json)
